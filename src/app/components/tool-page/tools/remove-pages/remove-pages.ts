...
import { PdfPageGalleryComponent } from "../../../pdf-workspace/pdf-page-gallery/pdf-page-gallery";
import {
  PdfService,
  PdfPage,
  PdfState,
      ...
      <div class="top-bar">
        <div class="top-bar-left">
          ...
          <div class="tool-badge">
            ...
            <span class="tool-name">Remove Pages</span>
          </div>
        </div>
        <div class="top-bar-right">
          <button
            class="process-btn"
            [disabled]="!pdfState || isProcessing || deletedCount === 0"
            (click)="process()"
            type="button"
          >
            @if (!isProcessing) {
            ...
            {{
              isProcessing
                ...
                : "Remove " +
                  (deletedCount > 0
                    ? deletedCount + " Page" + (deletedCount > 1 ? "s" : "")
                    : "Pages")
            }}
          </button>
        </div>
      ...
      <div class="content-area">
        ...
        @if (!pdfState && !isLoading) {
          ...
          >
            ...
            <p class="upload-hint">
              Click pages to mark them for removal · Processed in your browser
            </p>
          </div>
        }
        ...
        @if (pdfState && !isLoading) {
          <div class="settings-panel">
            <div class="settings-title">How it works</div>
            <p
              style="font-size:.85rem;color:var(--text-muted);font-weight:500;line-height:1.6"
            >
              Click any page to mark it for removal (red ✕ overlay). Click again
              to restore it. When you're ready, click
              <strong style="color:var(--text-main)">Remove Pages</strong> to
