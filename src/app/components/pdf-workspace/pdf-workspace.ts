...
import { PdfPageGalleryComponent } from "./pdf-page-gallery/pdf-page-gallery";

import { RemovePagesService } from "../../services/tools/remove-pages";
import { OrganizePdfService } from "../../services/tools/organize-pdf";
import { RotatePdfService } from "../../services/tools/rotate-pdf";
import { ExtractPagesService } from "../../services/tools/extract-pages";

@Component({
  selector: "app-pdf-workspace",
  standalone: true,
  imports: [CommonModule, RouterModule, PdfPageGalleryComponent],
  templateUrl: './pdf-workspace.html',
  styleUrls: ['./pdf-workspace.css'],
})
export class PdfWorkspaceComponent implements OnInit {
  ...
  async exportDocument() {
    ...
    try {
      ...
      const rawBytes = await this.storage.getWorkingCopy(
        this.state.activeDocument.id,
      );
      ...
      switch (this.state.activeTool) {
        case "remove":
          finalBlob = await this.removeService.process(
            rawBytes,
            this.state.pages,
          );
          break;
        case "organize":
          finalBlob = await this.organizeService.process(
            rawBytes,
            this.state.pages,
          );
          break;
        case "rotate":
          finalBlob = await this.rotateService.process(
            rawBytes,
            this.state.pages,
          );
          break;
        case "extract":
          finalBlob = await this.extractService.process(
            rawBytes,
            this.state.pages,
          );
          break;
