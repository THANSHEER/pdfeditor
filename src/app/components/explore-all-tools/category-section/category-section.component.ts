...
@Component({
  ...
  template: `
    @if (tools.length > 0) {
      <section
        [id]="category.id"
        class="max-w-7xl mx-auto px-6 py-10 border-b border-main"
      ...
      >
        ...
        >
          <div class="flex items-start gap-4">
            <div
              class="flex items-center justify-center w-12 h-12 rounded-2xl bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400"
            >
              <svg
                class="w-6 h-6"
            ...
            <div>
              <h2 class="text-2xl font-black text-main -tracking-wider">
                {{ category.name }}
              </h2>
          ...
          <span
            class="inline-block px-3 py-1 rounded-full bg-surface border border-main text-xs font-bold text-muted whitespace-nowrap"
